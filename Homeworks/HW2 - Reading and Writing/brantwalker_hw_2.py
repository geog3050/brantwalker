...
def import_data(filename='test_input.csv'):
    """
...
def fight(participant1, participant2, first2attack):
    ...
    while HP1>0 and HP2>0:
        if first2attack==1:
            damage=participant1[3]
            #HP is defending pokemon is reduced by the attackers damange
            # times the bonus
            HP2= HP2 - damage*attack_multiplier(participant1[1],participant2[1])

            #toggle first2attack
            first2attack=2

        elif first2attack==2:
            damage=participant2[3]
            HP1= HP1 - damage*attack_multiplier(participant2[1],participant1[1])

            first2attack=1

        else:
            print("First2attack is messed up")
...
def tournament(participants):
    ...
    for i in participants[:-1]:
        for j in participants[participants.index(i):]:
            if fight(i,j,1)[0]==1:
                wins[participants.index(i)] += 1
            elif fight(i,j,1)[0]==2:
                wins[participants.index(j)] += 1

            if fight(i,j,2)[0]==1:
                wins[participants.index(i)] += 1
            elif fight(i,j,2)[0]==2:
                wins[participants.index(j)] += 1
