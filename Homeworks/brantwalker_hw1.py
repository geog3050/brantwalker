...
mode = str(input("What are we calculating? 'min', 'max', or 'avg'?"))

count = int(input("How many values will you give me?"))

mylist = [] #Create a list, then ask for the correct number of values and
for i in range(count): # store them in the list. 
    nextvalue = float(input("What is the next value? (" + str(count-i) + " left) "))
    mylist.append(nextvalue)
...
#Sorting the list allows us to find min/max
mylist.sort()

if mode == 'min':
    print(mylist[0]) #first index of a sorted list
elif mode == 'max':
    print(mylist[len(mylist)-1]) #last value in a sorted list
else:
    num = 0
    for i in mylist:
        num += i #manually compute average
    print(num/count)
